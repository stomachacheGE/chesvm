...
% Initialize exemplar-SVMs and organize training set into calibration set and
% negative set.
%
% By Liangcheng Fu.
%
...
for i=1:length(datas)
    left = 1:length(datas);
    % Use images not from the positive class as negatives.
    left = left(left~=i);
...
for i=1:length(datas)
    ...
    for j=1:length(datas{i})
        ...
        models{i}{j} = model;  
        % Randomize the negative set.
        myRandomize;
        ordering = randperm(length(tmp_neg_sets{i}.img_filers));
        %take 1/10 of iamges per class for calibration 
